...
# test_seasons = [2010]
regions = ["X"]
# regions = [f"Region {i}" for i in range(1,11)] + ["X"]
...
def evaluate(sample=True, dtype="test", week_ahead=week_ahead):
    with torch.no_grad():
        emb_model.eval()
        emb_model_full.eval()
        fnp_model.eval()
        ...
        if dtype == "val":
            curr_x, curr_meta, curr_lens = val_x, val_meta, val_lens.copy()
        elif dtype == "test":
            curr_x, curr_meta, curr_lens = test_x, test_meta, test_lens.copy()
        elif dtype == "train":
            curr_x, curr_meta, curr_lens = train_x, train_meta, train_lens.copy()
        elif dtype == "all":
            curr_x, curr_meta, curr_lens = train_x_, train_meta_, train_lens_.copy()
        else:
            raise ValueError("Incorrect dtype")
        for _ in range(week_ahead):
            curr_mask = create_mask(curr_lens)
            x_embeds = emb_model.forward_mask(
                curr_x.transpose(1, 0), curr_meta, curr_mask
            )
            y_pred, _, vars, _, _, _, _ = fnp_model.predict(
                x_embeds, full_embeds, full_y, sample=sample
            )
            curr_z = float_tensor(np.zeros((curr_x.shape[0], 1, 1)))
            curr_x = torch.cat([curr_x, curr_z], 1)
            curr_x[np.arange(curr_x.shape[0]), curr_lens] = y_pred
            curr_lens += 1
...
yp = np.array([evaluate(True)[1] for _ in tqdm(range(10000))])
...
plt.legend()
plt.title(f"RMSE: {e}")
plt.savefig(f"plots/Test_regress_{week_ahead}_{model_num}.png")
dt = {
    "rmse": e,
    "target": yt,
    "pred": yp,
...
save_data(dt, f"./saves/regress_{week_ahead}_{model_num}_test.pkl")

e, yp, yt, vars, _, _ = evaluate(True, dtype="val")
yp = np.array([evaluate(True, dtype="val")[1] for _ in tqdm(range(1000))])
yp, vars = np.mean(yp, 0), np.var(yp, 0)
e = np.mean((yp - yt) ** 2)
dev = np.sqrt(vars) * 1.95
plt.figure(5)
...
plt.legend()
plt.title(f"RMSE: {e}")
plt.savefig(f"plots/Val_regress_{week_ahead}_{model_num}.png")

e, yp, yt, vars, fem, tem = evaluate(True, dtype="all")
yp = np.array([evaluate(True, dtype="all")[1] for _ in tqdm(range(40))])
yp, vars = np.mean(yp, 0), np.var(yp, 0)
e = np.mean((yp - yt) ** 2)
dev = np.sqrt(vars) * 1.95
plt.figure(6)
...
save_data(dt, f"./saves/regress2_{week_ahead}_{model_num}_train.pkl")
